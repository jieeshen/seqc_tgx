#
#   Module: lco_lco_rbf_ig.py
#	Author: Jie Shen  @ NCTR/FDA
#	Date: Sep. 4, 2012.
#
#   updated: Sep.6 2012. Updated the modelbuild_lco. re-generate the compound list according to training set.


#   Description:
#       This module is based on the modeling_rbf_ig_v2.py. The purpose is to do "leave compound out" cross validation
#       on the TGX data set. The inner parameters were also determined using LCO.
############################
...
def process_options(argv=sys.argv):
    ...
    global complistfile, complist, compmapmatrix, mapsamples

    usage = """\
Usage: modeling_rbf_ig.py [-n begin,end,step] [-log2c begin,end,step] [-log2g begin,end,step] [-v fold]
-i labelfile -j datafile -l complistfile -d outputDIR"""
...
def modelbuild(training, testing):
    # training and testing are the matrix of the data sets without the title.
...
def modelbuild_lco(training,testing):
# training and testing are the matrix of the data sets without the title.
...
# complist, compmatrix
    ...
    tempcomplist=[]
    for dataline in training:
        id=mapsamples.index(dataline[0])
        tempcomplist.append(compmapmatrix[id][0])
    newcomplist=list(set(tempcomplist))
    ...
    for n in range_f(n_begin,n_end,n_step):
        for c in permute_sequence(range_f(c_begin, c_end, c_step)):
            for g in permute_sequence(range_f(g_begin, g_end, g_step)):
                ...
                acc_list=[]
                #print("n=%d, c=%d, g=%d\n" %(n,c,g))
                for innerrun in range(0,len(newcomplist)):   #perform leave compound out cross validation...
...
# Give the default parameters
    ...
    for comp in complist:
        ...
        results=modelbuild_lco(training,testing)
        params=results[1]
        out_feature.write("(n=%d,c=%d,g=%d)\t" % (params[0],params[1],params[2])),
        for fno in results[2]:
            out_feature.write(str(featuretitles[fno])+" "),
        out_feature.write("\tMAX Average Accurate: %f\n" % results[0])
        for i in range(0,len(testing)):
            out_pred.write("%s\t%d\t%f\n" % (testing[i][0],results[3][i],results[4][i]))
